/*!
 * ====================================================
 * Kity Minder Core - v1.4.50 - 2019-07-12
     ...
     */
        ...
        var TextRenderer = kity.createClass("TextRenderer", {
            ...
            update: function(textGroup, node) {
                ...
                var textArr = [ " " ];
                var autoLine = new kity.Text();
                var textGroupLength = textGroup.getItems().length;
                textGroup.addItem(autoLine);
                if (nodeText) {
                    var tmp = "";
                    var charArr = nodeText.split("");
                    textArr = [];
                    for (var index in charArr) {
                        ...
                        if (charArr[index] === "\n") {
                            ...
                            autoLine.setContent(tmp);
                            if (autoLine.getBoundaryBox().width >= 500) {
                                textArr.push(tmp);
                                tmp = "";
                            }
                        }
         ...
         */
            ...
            var VideoRenderer = kity.createClass("VideoRenderer", {
                ...
                update: function(image, node, box) {
                    ...
                    var x = box.cx - size.width / 2;
                    var y = box.y - size.height - spaceTop;
                    image.setData("video", node.getData("video")).setUrl(url).setX(x | 0).setY(y | 0).setWidth(size.width | 0).setHeight(size.height | 0);
                    return new kity.Box(x | 0, y | 0, size.width | 0, size.height | 0);
                }
            });
            return {
