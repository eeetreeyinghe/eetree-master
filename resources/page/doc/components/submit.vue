      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <a :href="'/doc/edit/' + scope.row.id" class="flex flex-center-v">
            ...
            <el-tag size="mini" type="info" effect="dark" v-if="scope.row.status === 1">审核中</el-tag>
            <el-tag size="mini" type="danger" effect="dark" v-if="scope.row.status === 8">审核未通过</el-tag>
            <el-tag size="mini" type="success" effect="dark" v-if="scope.row.status === 9">审核通过</el-tag>
          </a>
        </template>
      </el-table-column>
      <el-table-column
        ...
        width="160">
        <template slot-scope="scope">
          <div class="created-time-el">{{ scope.row.submit_at }}</div>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="bottom">
            <ul class="flex-ul-center">
              ...
              <li v-if="scope.row.status === 8" @click="showReason(scope.row)">
                <i class="fa fa-question-circle-o" aria-hidden="true"></i>未通过原因
              </li>
            </ul>
...
<script>
import { getPublishs } from '@/js/api/doc'
...
export default {
  ...
  methods: {
    ...
    showReason(row) {
      this.$alert(row.review_remarks, '原因', {
