...
class ResetPasswordController extends Controller
{
    ...
    public function resetByMobile(Request $request)
    {
        if (empty($request->password)) {
            if (!SmsHelper::verify($request->input('mobile'), $request->input('verify_code'), 'forget')) {
                return back()
                    ->withInput($request->only('mobile'))
                    ->withErrors(['verify_code' => '手机验证码错误']);
            }
    ...
    protected function resetBroker(array $credentials, $callback)
    {
        ...
        if (!SmsHelper::verify($credentials['mobile'], $credentials['verify_code'], 'forget')) {
            return '手机验证码过期';
        }
        $pass = $credentials['password'];
        call_user_func($callback, $user, $pass);

        SmsHelper::clear($credentials['mobile']);
